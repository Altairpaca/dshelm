...
async function mountPreset(agentCtx, preset) {
	...
	// Mount guard: a row whose apply never settles (sync loop or a never-
	// resolving async effect) would otherwise hang session.create forever
	// with no server log — the observed "PTC preset session creation hang".
	// Time out, name the still-pending rows, and fail loud instead.
	...
	const timeout = new Promise((_, reject) => {
		mountTimer = setTimeout(() => {
			...
			for (const entry of handle.entries()) {
				if (entry.disabled) continue;
				if (entry._initTask) pending.push(`${entry.options.id} (${entry.options.name}): import in progress`);
				const fiber = entry.fiber;
				if (fiber?.inertia) pending.push(`${entry.options.id} (${entry.options.name}): apply in progress`);
				else if (fiber === void 0) pending.push(`${entry.options.id} (${entry.options.name}): never started`);
			}
			reject(new Error(`preset "${preset.id}" mount timed out after ${MOUNT_TIMEOUT_MS}ms; still pending: ${pending.length ? pending.join("; ") : "(none enumerated)"}`));
		}, MOUNT_TIMEOUT_MS);
	});
	try {
		await Promise.race([handle.await(), timeout]);
