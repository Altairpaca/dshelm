...
var FrameQueue = class {
	...
	push(item) {
		...
		if (this.buffer.length > this.maxSize) this.dropOverflow();
		this.waiter?.();
	}
	dropOverflow() {
		// Mux-flood safety net: bound queue memory under extreme token deluge.
		// Token frames are drop-safe on the client (a seq gap triggers a
		// history repull), so oldest assistant/chunk frames go first; other
		// frames are only dropped as a last resort.
		let dropped = 0;
		while (this.buffer.length > this.maxSize && dropped < 64) {
			const idx = this.buffer.findIndex((f) => f?.payload?.type === "session/event" && f.payload.event?.type === "assistant/chunk");
			if (idx === -1) break;
	...
	return {
		...
		events: {
			mux(_request, signal) {
				...
				// ---- mux-flood coalescer ----
				// Providers stream token-sized deltas (assistant/chunk). Without
				// coalescing, every chunk of every session is pushed to every mux
				// connection, hundreds of frames per second, starving the browser
				// main thread (DSH #1316 family: UI freezes, New Session and
				// refresh stop responding while streams keep flowing).
				// Consecutive deltas of the same block are losslessly mergeable —
				// the client stitcher concatenates per (index, type) — so runs are
				// batched into one `session/events` frame flushed every
				// CHUNK_FLUSH_MS. Merged frames carry the run's LAST seq so the
				// client's gap check stays satisfied; the client applies batches
				// with the gap check bypassed. Non-chunk events are never delayed.
				const CHUNK_FLUSH_MS = 60;
				const chunkRuns = /* @__PURE__ */ new Map();
				const flushChunkRun = (run) => {
					const event = {
				...
				const scheduleChunkFlush = (run) => {
					if (run.timer !== void 0) return;
					run.timer = setTimeout(() => {
						run.timer = void 0;
						chunkRuns.delete(run.key);
						flushChunkRun(run);
					}, CHUNK_FLUSH_MS);
				};
				const coalesceChunk = (session, event) => {
					const data = event.data;
					const chunk = data.chunk;
					if (chunk.type !== "text-delta" && chunk.type !== "reasoning-delta" && chunk.type !== "tool-call-delta") return false;
					if (!Number.isInteger(data.turn) || !Number.isInteger(data.step) || !Number.isInteger(chunk.index)) return false;
					const key = `${session.id}\u0000${data.turn}\u0000${data.step}\u0000${chunk.index}\u0000${chunk.type}`;
					const run = chunkRuns.get(key);
					if (run === void 0) {
						const fresh = {
							key,
							sessionId: session.id,
						...
						chunkRuns.set(key, fresh);
						scheduleChunkFlush(fresh);
						return true;
					}
					const base = run.chunk;
					if (chunk.type === "text-delta" || chunk.type === "reasoning-delta") {
						base.text += chunk.text;
					} else if (base.argumentsDelta !== void 0 && chunk.argumentsDelta !== void 0) {
						base.argumentsDelta += chunk.argumentsDelta;
					} else {
						// Shape mismatch: flush the pending run immediately and let
						// this event flow through the ordinary path unchanged.
				...
				const flushAllChunkRuns = () => {
					for (const run of chunkRuns.values()) {
						clearTimeout(run.timer);
						flushChunkRun(run);
					}
					chunkRuns.clear();
				...
				const disposers = [
					ctx.on("session/event", (session, event) => {
						...
						if (event.type === "assistant/chunk") {
							// Any unexpected chunk shape falls through to the raw
							// path (never let a malformed event crash the listener).
							try {
								if (coalesceChunk(session, event)) return;
							} catch {}
						}
						// Anything that is not a mergeable delta interrupts pending
						// runs: flush them first so wire order matches event order.
						flushAllChunkRuns();
				...
				return queue.iterate(signal, () => {
					...
					for (const run of chunkRuns.values()) clearTimeout(run.timer);
					chunkRuns.clear();
