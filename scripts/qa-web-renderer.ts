...
<html lang="en">
  ...
  <body>
    ...
    <script type="module">
      ...
      apply({
        dshelmPolicy: { snapshot: () => snapshot },
...
try {
  ...
  await narrow.locator('[data-dshelm-inspector]').waitFor()
  const narrowWidth = await narrow.locator('[data-dshelm-control-plane]').evaluate(
    element => element.getBoundingClientRect().width,
  )
  const traceItems = await narrow.locator('[data-dshelm-inspector] li').count()
...
if (!passed) {
  throw new Error('DSHelm browser QA failed')
}
